...
- Регистрация и вход:
  ...
  - `GET /login`
  - `GET /register`
- Plugin storefront интеграция с Remnashop backend:
  - `POST /api/plugin/auth/telegram/callback`
  - `GET /api/plugin/me`
  - `GET /api/plugin/subscription`
  - `POST /api/plugin/checkout`
...
Переменные окружения для plugin storefront (бот как backend):

- `REMNASHOP_API_URL`
- `REMNASHOP_SERVICE_TOKEN`
- `REMNASHOP_SERVICE_HEADER_NAME`
- `REMNASHOP_SERVICE_HEADER_PREFIX`
- `REMNASHOP_USERS_BY_TELEGRAM_PATH`
- `REMNASHOP_USERS_BY_EMAIL_PATH`
- `REMNASHOP_USER_BY_ID_PATH`
- `REMNASHOP_SUBSCRIPTION_BY_USER_PATH`
- `REMNASHOP_CHECKOUT_PATH`
- `REMNASHOP_PLANS_PATH`
