...
## Супер-простой запуск на VPS через GHCR (бот уже работает)

1. Клонируйте репо в `/opt/vpn-shop` и заполните `.env`:
   - `cp .env.example .env`
2. Первый запуск:
   - `bash scripts/ghcr-install-addon.sh`
3. Обновление до последней версии:
   - `bash scripts/ghcr-update-addon.sh`

Что делает скрипт установки:
- создает `docker-compose.override.yml` с `app: ghcr.io/d3myasha/vpn-shop:latest` и портом `3001:3000`,
- поднимает стек,
- применяет миграции и seed,
- проверяет `http://127.0.0.1:3001/api/health`.

Важно:
- если GHCR приватный, сначала выполните `docker login ghcr.io -u <github_user>`,
- если Caddy в отдельном compose, проксируйте домен на `127.0.0.1:3001`.
