...
### Self-hosted SMTP (без внешних сервисов)

Оптимальный для небольшого VPS вариант: `Postfix` как локальный SMTP на хосте, а приложение в Docker отправляет в него.

Плюсы:
- минимальная нагрузка по CPU/RAM;
- не зависит от внешнего SMTP-провайдера;
- при смене домена нужно обновить DNS + переменные `SMTP_HELO_NAME`/`SMTP_FROM`.

Приложение поддерживает режим без SMTP-авторизации:
- `SMTP_HOST=host.docker.internal`
- `SMTP_PORT=25`
- `SMTP_AUTH_ENABLED=false`

Команды установки Postfix на VPS (Ubuntu):
...
1. `Internet Site`
2. System mail name: ваш домен, например `mail.example.com`
...
Рекомендуемые DNS записи для доставляемости:
- `A` для `mail.<ваш-домен>`
- `MX` на `mail.<ваш-домен>`
- `SPF` (`TXT`) с вашим сервером
- `DKIM` (через OpenDKIM/Postfix)
- `DMARC` (`TXT`)

Без корректных DNS (особенно SPF/DKIM/DMARC) письма часто попадают в спам.
