...
- Инфраструктура для VPS:
  ...
  - `docker-compose.one-vps.yml` (bot + shop + postgres_bot + postgres_shop + redis + caddy)
  - `caddy/docker-compose.yml` (отдельный reverse proxy)
  - `caddy/Caddyfile` (TLS + проксирование на Next.js)
  - `caddy/Caddyfile.one-vps` (TLS + проксирование `shop` и `bot` в одном стеке)
...
Переменные для единого one-VPS стека:

- `SHOP_DOMAIN`
- `BOT_DOMAIN`
- `REMNASHOP_BOT_IMAGE`
- `SHOP_DB_NAME`, `SHOP_DB_USER`, `SHOP_DB_PASSWORD`
- `BOT_DB_NAME`, `BOT_DB_USER`, `BOT_DB_PASSWORD`
- `BOT_DB_READONLY_USER`, `BOT_DB_READONLY_PASSWORD`
- переменные самого бота (`APP_CRYPT_KEY`, `BOT_TOKEN`, `BOT_SECRET_TOKEN`, `BOT_OWNER_ID`, `BOT_SUPPORT_USERNAME`, `REMNAWAVE_TOKEN`, и т.д.)
...
## Единый VPS: бот + сайт в одном compose

1. Подготовьте `.env` на базе `.env.example` и заполните:
   - домены `SHOP_DOMAIN` и `BOT_DOMAIN`
   - переменные бота (минимум: `APP_CRYPT_KEY`, `BOT_TOKEN`, `BOT_SECRET_TOKEN`, `BOT_OWNER_ID`)
   - пароли БД (`SHOP_DB_*`, `BOT_DB_*`, `BOT_DB_READONLY_*`)
2. Запустите единый стек:
   - `docker compose -f docker-compose.one-vps.yml up -d --build`
3. Для shop БД примените миграции:
   - `docker compose -f docker-compose.one-vps.yml exec shop npm run prisma:deploy`
   - `docker compose -f docker-compose.one-vps.yml exec shop npm run prisma:seed`

В этом режиме:
- сайт читает core-данные бота напрямую из БД бота через `REMNASHOP_DATABASE_URL` (read-only),
- покупка/смена/продление идут через Telegram deep-link (`start=plan_<public_code>`),
- сайт не пишет в core-таблицы бота.
