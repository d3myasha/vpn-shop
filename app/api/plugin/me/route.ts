...
import { getBotUserById, isRemnashopConfigured, RemnashopAdapterError } from "@/lib/remnashop-adapter";
...
export async function GET() {
  ...
  try {
    const botUser = await getBotUserById(identity.botUserId);
    return NextResponse.json({ user, botIdentity: identity, botUser });
  } catch (error) {
